...
const sketch = (p) => {
    ...
    function inicializarIslas() {
        ...
        let configIslas = [
            { nombre: 'Roja', color: COLORES_FIGMA.rojo, dist: 420, angulo: angulosBase[0] },
            { nombre: 'Verde', color: COLORES_FIGMA.verde, dist: 690, angulo: angulosBase[1] },
            { nombre: 'Azul', color: COLORES_FIGMA.azul, dist: 920, angulo: angulosBase[2] },
            { nombre: 'Morada', color: COLORES_FIGMA.morado, dist: 1150, angulo: angulosBase[3] }
        ];
        ...
        for (let config of configIslas) {
            let anguloFinal = config.angulo + p.random(-0.25, 0.25);
            let distFinal = config.dist + p.random(-30, 30);
            islas.push({
    ...
    // AQUÍ ESTÁ TU RESPUESTA: Esto le ordena a p5_js meterse estrictamente dentro del contenedor HTML corregido
    p.setup = () => {
            ...
            if (n.tipo === 'vacio' && p.dist(mouseMundialX, mouseMundialY, n.x, n.y) < tamanoHex) {
                ...
                for (let isla of islas) {
                    if (!isla.alcanzada) {
                        ...
                        if (p.dist(n.x, n.y, isla.x, isla.y + flotacionActual + 60) < 85 || p.dist(n.x, n.y, isla.x, isla.y + flotacionActual - 50) < 75) {
    ...
    function ramificar(nodoPadre) {
        ...
        let ramasCreadas = 0, intentosAngulo = 0;
        while (ramasCreadas < cantidadRamas && intentosAngulo < 12) {
            let anguloPropuesto = anguloBase + (intentosAngulo * (p.TWO_PI / 6)) + p.random(-0.05, 0.05);
            ...
            for (let existente of nodos) { if (p.dist(nuevoX, nuevoY, existente.x, existente.y) < tamanoHex * 2.1) { superpuesto = true; break; } }
            if (!superpuesto) {
                let nuevoNodo = { id: nodos.length + 1, x: nuevoX, y: nuevoY, tipo: 'vacio', color: '#000000', hijosMax: 0, padreX: nodoPadre.x, padreY: nodoPadre.y };
                nodos.push(nuevoNodo); lineas.push({ p1: nodoPadre, p2: nuevoNodo }); ramasCreadas++;
            }
            intentosAngulo++;
        }
    }
    ...
    p.windowResized = () => { p.resizeCanvas(p.windowWidth, 500); };
    reiniciarJuegoTotal = () => { configurarEntornoInicial(); };
};

// Arrancar la instancia de p5
